...
as.gam <- function(object) {
  ...
  g1 <- mgcv::gam(G = object $ Gsetup)

  # recheck for rank deficiency
  qr.G <- qr(object $ G)
  rank.deficient <- qr.G $ pivot[abs(diag(qr.G $ qr)) < 1e-7]
  whichkeep <- -rank.deficient
  if (!length(whichkeep)) whichkeep <- 1:length(object $ coefficients)
  redudant <- names(g1 $ coefficients[-1 * whichkeep])
  if (length(redudant)) {
    warning("there is some reduandancy in the model specification.  This may be fine, but best to check.")
  }
...
efp3 <- function(formula, data = NULL, pass, id, offset = NULL, Q = NULL,
  ...
  # dont remove redundant / collinear parameters if Q is specified
  if (is.null(Q)) {
    qr.G <- qr(G)
    rank.deficient <- qr.G $ pivot[abs(diag(qr.G $ qr)) < 1e-7]
    if (length(rank.deficient)) {
  ...
  if (!verbose) {
    tmp <-
      capture.output(
        opt <- rstan::optimizing(.efEnv $ stanmod3, data = standat, algorith = "BFGS", hessian = hessian, verbose = verbose, init = init)
      )
  } else {
    opt <- rstan::optimizing(.efEnv $ stanmod3, data = standat, algorith = "BFGS", hessian = hessian, verbose = verbose, init = init)
  }
